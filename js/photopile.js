...
var photopile = (function() {
    ...
    // Photo container
    var fadeDuration      = 50;        // speed at which photo fades (ms)
    var pickupDuration    = 100;        // speed at which photo is picked up & put down (ms)
    var photoZIndex       = 100;        // z-index (show above all)
    var photoBorder       = 5;         // border width around fullsize image
        ...
        pickup : function( thumbnail ) {
            ...
            if ( self.isPickedUp ) {
                ...
                self.loadImage( thumb.getActiveImgSrc(), function() {
                    ...
                    self.image.bind('click', function() { self.openArticle(); }); // bind open article event to the image
                });
            }
        }, // pickup

		// Open article in new window.
		openArticle : function ( callback )
		{
			var imgSrc = thumb.getActiveImgSrc();
			var strArray = imgSrc.split('/');
			window.open("article.php?id=" + strArray[2]);
			self.putDown();
		},
